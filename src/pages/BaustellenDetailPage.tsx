...
import { formEnhancements } from '@/config/form-enhancements/Baustellen';
import { evalComputed } from '@/config/form-enhancements/types';

export default function BaustellenDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [record, setRecord] = useState<Baustellen | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const list = await LivingAppsService.getBaustellen();
  ...
  async function handleUpdate(fields: Baustellen['fields']) {
    if (!record) return;
    await LivingAppsService.updateBaustellenEntry(record.record_id, fields);
    await loadData();
    setEditing(false);
  }
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteBaustellenEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/baustellen');
  }
  ...
  if (!record) {
    return (
      <RecordViewEmpty
        ...
        action={
          <Button variant="ghost" onClick={() => navigate('/baustellen')}>
  ...
  return (
    <RecordView
      onBack={() => navigate('/baustellen')}
      onEdit={() => setEditing(true)}
      backLabel="Zurück"
      editLabel="Bearbeiten"
    >
      <RecordHeader title={record.fields.name ?? 'Baustellen'} />
      ...
      <RecordSection title="Details" cols={2}>
        <RecordField label="Baustellenname" value={record.fields.name} format="text" />
        ...
        <RecordField label="Auftraggeber" value={record.fields.auftraggeber} format="text" />
        <RecordField label="Notizen" value={record.fields.notizen} format="longtext" className="md:col-span-2" />
      </RecordSection>

      <RecordAttachments appId={APP_IDS.BAUSTELLEN} recordId={record.record_id} />
