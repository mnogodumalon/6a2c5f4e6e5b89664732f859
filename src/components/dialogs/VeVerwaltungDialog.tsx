...
import type { VeVerwaltung, Baustellen, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/VeVerwaltung';
...
interface VeVerwaltungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: VeVerwaltung['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Omit<VeVerwaltung['fields'], 've_status'> & {
    ve_status?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  ve_status: LOOKUP_OPTIONS['ve_verwaltung']?.['ve_status'] ?? [],
};
...
export function VeVerwaltungDialog({ open, onClose, onSubmit, defaultValues, recordId, baustellenList, enablePhotoScan = true, enablePhotoLocation = true }: VeVerwaltungDialogProps) {
  const [fields, setFields] = useState<Partial<VeVerwaltung['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<VeVerwaltung['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 've_verwaltung');
      await onSubmit(clean as VeVerwaltung['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "baustelle": string | null, // Display name from Baustellen (see <available-records>)\n  "ve_nummer": string | null, // VE-Nummer\n  "ausstellungsdatum": string | null, // YYYY-MM-DD\n  "ablaufdatum": string | null, // YYYY-MM-DD\n  "erinnerungsdatum": string | null, // YYYY-MM-DD\n  "ve_status": LookupValue | null, // Status der VE (select one key: "beantragt" | "genehmigt" | "abgelaufen" | "widerrufen") mapping: beantragt=Beantragt, genehmigt=Genehmigt, abgelaufen=Abgelaufen, widerrufen=Widerrufen\n  "behoerde": string | null, // Zuständige Behörde\n  "ansprechpartner_vorname": string | null, // Vorname Ansprechpartner\n  "ansprechpartner_nachname": string | null, // Nachname Ansprechpartner\n  "ansprechpartner_telefon": string | null, // Telefon Ansprechpartner\n  "ve_notizen": string | null, // Notizen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<VeVerwaltung['fields']>;
      });
  ...
  const DIALOG_INTENT = defaultValues ? 'VE-Verwaltung bearbeiten' : 'VE-Verwaltung hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    've_nummer': (
      <div key="ve_nummer" className="space-y-1.5">
    ...
    'ablaufdatum': (
      <div key="ablaufdatum" className="space-y-1.5">
    ...
    'erinnerungsdatum': (
      <div key="erinnerungsdatum" className="space-y-1.5">
    ...
    've_status': (
      <div key="ve_status" className="space-y-1.5">
    ...
    'behoerde': (
      <div key="behoerde" className="space-y-1.5">
    ...
    'dokumente': (
      <div key="dokumente" className="space-y-1.5">
        <Label htmlFor="dokumente">Dokumente und Genehmigungen</Label>
        {fields.dokumente ? (
    ...
    've_notizen': (
      <div key="ve_notizen" className="space-y-1.5">
  ...
  const FIELD_LABELS: Record<string, string> = {"baustelle": "Baustelle", "ve_nummer": "VE-Nummer", "ausstellungsdatum": "Ausstellungsdatum", "ablaufdatum": "Ablaufdatum", "erinnerungsdatum": "Erinnerungsdatum", "ve_status": "Status der VE", "behoerde": "Zuständige Behörde", "ansprechpartner_vorname": "Vorname Ansprechpartner", "ansprechpartner_nachname": "Nachname Ansprechpartner", "ansprechpartner_telefon": "Telefon Ansprechpartner", "dokumente": "Dokumente und Genehmigungen", "ve_notizen": "Notizen"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.VE_VERWALTUNG} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
