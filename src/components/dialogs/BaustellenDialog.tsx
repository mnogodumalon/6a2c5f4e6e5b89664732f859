...
import type { Baustellen, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Baustellen';
...
interface BaustellenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Baustellen['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Omit<Baustellen['fields'], 'status'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  status: LOOKUP_OPTIONS['baustellen']?.['status'] ?? [],
};
...
export function BaustellenDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: BaustellenDialogProps) {
  const [fields, setFields] = useState<Partial<Baustellen['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Baustellen['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'baustellen');
      await onSubmit(clean as Baustellen['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Baustellen['fields']>;
      });
      if (gps) {
  ...
  const DIALOG_INTENT = defaultValues ? 'Baustellen bearbeiten' : 'Baustellen hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'auftraggeber': (
      <div key="auftraggeber" className="space-y-1.5">
  ...
  const FIELD_LABELS: Record<string, string> = {"name": "Baustellenname", "beschreibung": "Beschreibung", "status": "Status", "startdatum": "Startdatum", "enddatum": "Geplantes Enddatum", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "standort": "GPS-Standort", "zustaendige_person": "Zuständige Person", "auftraggeber": "Auftraggeber", "notizen": "Notizen"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.BAUSTELLEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
