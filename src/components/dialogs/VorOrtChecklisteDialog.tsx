...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/VorOrtCheckliste';
...
interface VorOrtChecklisteDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: VorOrtCheckliste['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: VorOrtCheckliste['fields'];
...
export function VorOrtChecklisteDialog({ open, onClose, onSubmit, defaultValues, recordId, baustellenList, enablePhotoScan = true, enablePhotoLocation = true }: VorOrtChecklisteDialogProps) {
  const [fields, setFields] = useState<Partial<VorOrtCheckliste['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<VorOrtCheckliste['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'vor_ort_checkliste');
      await onSubmit(clean as VorOrtCheckliste['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "baustelle": string | null, // Display name from Baustellen (see <available-records>)\n  "pruefzeitpunkt": string | null, // YYYY-MM-DDTHH:MM\n  "pruefer": string | null, // Prüfer\n  "absperrungen_korrekt": boolean | null, // Sind die Absperrungen korrekt?\n  "parkverbote_eingerichtet": boolean | null, // Sind die Parkverbote eingerichtet?\n  "anwohner_informiert": boolean | null, // Wurden Anwohner informiert?\n  "grabengroesse_korrekt": boolean | null, // Entspricht die Grabengröße dem Plan?\n  "bemerkungen": string | null, // Bemerkungen und Abweichungen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<VorOrtCheckliste['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Vor-Ort-Checkliste bearbeiten' : 'Vor-Ort-Checkliste hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'pruefzeitpunkt': (
      <div key="pruefzeitpunkt" className="space-y-1.5">
    ...
    'absperrungen_korrekt': (
      <div key="absperrungen_korrekt" className="space-y-1.5">
        <Label htmlFor="absperrungen_korrekt">Sind die Absperrungen korrekt?</Label>
        <div className="flex items-center gap-2 pt-1">
          <Checkbox
            id="absperrungen_korrekt"
            checked={!!fields.absperrungen_korrekt}
            onCheckedChange={(v) => setFields(f => ({ ...f, absperrungen_korrekt: !!v }))}
          />
          <Label htmlFor="absperrungen_korrekt" className="font-normal">Sind die Absperrungen korrekt?</Label>
        </div>
      </div>
    ),
    'parkverbote_eingerichtet': (
      <div key="parkverbote_eingerichtet" className="space-y-1.5">
        <Label htmlFor="parkverbote_eingerichtet">Sind die Parkverbote eingerichtet?</Label>
        <div className="flex items-center gap-2 pt-1">
          <Checkbox
            id="parkverbote_eingerichtet"
            checked={!!fields.parkverbote_eingerichtet}
            onCheckedChange={(v) => setFields(f => ({ ...f, parkverbote_eingerichtet: !!v }))}
          />
          <Label htmlFor="parkverbote_eingerichtet" className="font-normal">Sind die Parkverbote eingerichtet?</Label>
        </div>
      </div>
    ),
    'anwohner_informiert': (
      <div key="anwohner_informiert" className="space-y-1.5">
        <Label htmlFor="anwohner_informiert">Wurden Anwohner informiert?</Label>
        <div className="flex items-center gap-2 pt-1">
          <Checkbox
            id="anwohner_informiert"
            checked={!!fields.anwohner_informiert}
            onCheckedChange={(v) => setFields(f => ({ ...f, anwohner_informiert: !!v }))}
          />
          <Label htmlFor="anwohner_informiert" className="font-normal">Wurden Anwohner informiert?</Label>
        </div>
      </div>
    ),
    'grabengroesse_korrekt': (
      <div key="grabengroesse_korrekt" className="space-y-1.5">
        <Label htmlFor="grabengroesse_korrekt">Entspricht die Grabengröße dem Plan?</Label>
        <div className="flex items-center gap-2 pt-1">
          <Checkbox
            id="grabengroesse_korrekt"
            checked={!!fields.grabengroesse_korrekt}
            onCheckedChange={(v) => setFields(f => ({ ...f, grabengroesse_korrekt: !!v }))}
          />
          <Label htmlFor="grabengroesse_korrekt" className="font-normal">Entspricht die Grabengröße dem Plan?</Label>
        </div>
      </div>
    ),
    'bemerkungen': (
  ...
  const FIELD_LABELS: Record<string, string> = {"baustelle": "Baustelle", "pruefzeitpunkt": "Datum und Uhrzeit der Prüfung", "pruefer": "Prüfer", "absperrungen_korrekt": "Sind die Absperrungen korrekt?", "parkverbote_eingerichtet": "Sind die Parkverbote eingerichtet?", "anwohner_informiert": "Wurden Anwohner informiert?", "grabengroesse_korrekt": "Entspricht die Grabengröße dem Plan?", "bemerkungen": "Bemerkungen und Abweichungen"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.VOR_ORT_CHECKLISTE} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
