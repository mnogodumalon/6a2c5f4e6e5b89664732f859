...
import type { Leitungsauskunft, Baustellen, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Leitungsauskunft';
...
interface LeitungsauskunftDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Leitungsauskunft['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Omit<Leitungsauskunft['fields'], 'hausanschluss_typ' | 'spannungsebene'> & {
    hausanschluss_typ?: LookupValue | string;
    spannungsebene?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  hausanschluss_typ: LOOKUP_OPTIONS['leitungsauskunft']?.['hausanschluss_typ'] ?? [],
  spannungsebene: LOOKUP_OPTIONS['leitungsauskunft']?.['spannungsebene'] ?? [],
};
...
export function LeitungsauskunftDialog({ open, onClose, onSubmit, defaultValues, recordId, baustellenList, enablePhotoScan = true, enablePhotoLocation = true }: LeitungsauskunftDialogProps) {
  const [fields, setFields] = useState<Partial<Leitungsauskunft['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Leitungsauskunft['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'leitungsauskunft');
      await onSubmit(clean as Leitungsauskunft['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "baustelle": string | null, // Display name from Baustellen (see <available-records>)\n  "hausanschluss_info": string | null, // Hausanschluss-Informationen\n  "hausanschluss_typ": LookupValue | null, // Hausanschluss-Typ (select one key: "strom" | "gas" | "wasser" | "telekommunikation" | "fernwaerme") mapping: strom=Strom, gas=Gas, wasser=Wasser, telekommunikation=Telekommunikation, fernwaerme=Fernwärme\n  "spannungsebene": LookupValue | null, // Spannungsebene (select one key: "niederspannung" | "mittelspannung" | "hochspannung") mapping: niederspannung=Niederspannung, mittelspannung=Mittelspannung, hochspannung=Hochspannung\n  "muffentyp": string | null, // Muffentyp\n  "material": string | null, // Material\n  "leitungsfuehrung": string | null, // Leitungsführung und Notizen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Leitungsauskunft['fields']>;
      });
      ...
      if (file && (file.type.startsWith('image/') || file.type === 'application/pdf')) {
        try {
          ...
          setFields(prev => ({ ...prev, leitungsplaene: fileUrl }));
        } catch (uploadErr) {
  ...
  const DIALOG_INTENT = defaultValues ? 'Leitungsauskunft bearbeiten' : 'Leitungsauskunft hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'hausanschluss_info': (
      <div key="hausanschluss_info" className="space-y-1.5">
    ...
    'hausanschluss_typ': (
      <div key="hausanschluss_typ" className="space-y-1.5">
    ...
    'spannungsebene': (
      <div key="spannungsebene" className="space-y-1.5">
    ...
    'muffentyp': (
      <div key="muffentyp" className="space-y-1.5">
    ...
    'leitungsfuehrung': (
      <div key="leitungsfuehrung" className="space-y-1.5">
    ...
    'leitungsplaene': (
      <div key="leitungsplaene" className="space-y-1.5">
        <Label htmlFor="leitungsplaene">Leitungspläne und Unterlagen</Label>
        {fields.leitungsplaene ? (
          <div className="flex items-center gap-3 rounded-lg border p-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm truncate text-foreground">{fields.leitungsplaene.split("/").pop()}</p>
              <div className="flex gap-2 mt-1">
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        setFields(f => ({ ...f, leitungsplaene: fileUrl }));
                ...
                <button
                  ...
                  onClick={() => setFields(f => ({ ...f, leitungsplaene: undefined }))}
                >
                  Entfernen
                </button>
              </div>
  ...
  const FIELD_LABELS: Record<string, string> = {"baustelle": "Baustelle", "hausanschluss_info": "Hausanschluss-Informationen", "hausanschluss_typ": "Hausanschluss-Typ", "spannungsebene": "Spannungsebene", "muffentyp": "Muffentyp", "material": "Material", "leitungsfuehrung": "Leitungsführung und Notizen", "leitungsplaene": "Leitungspläne und Unterlagen"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.LEITUNGSAUSKUNFT} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
