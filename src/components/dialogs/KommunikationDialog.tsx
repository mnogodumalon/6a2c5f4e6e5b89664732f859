...
import type { Kommunikation, Baustellen, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Kommunikation';
...
interface KommunikationDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Kommunikation['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Omit<Kommunikation['fields'], 'kontaktart'> & {
    kontaktart?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  kontaktart: LOOKUP_OPTIONS['kommunikation']?.['kontaktart'] ?? [],
};
...
export function KommunikationDialog({ open, onClose, onSubmit, defaultValues, recordId, baustellenList, enablePhotoScan = true, enablePhotoLocation = true }: KommunikationDialogProps) {
  const [fields, setFields] = useState<Partial<Kommunikation['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Kommunikation['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'kommunikation');
      await onSubmit(clean as Kommunikation['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "baustelle": string | null, // Display name from Baustellen (see <available-records>)\n  "kontakt_vorname": string | null, // Vorname Kontaktperson\n  "kontakt_nachname": string | null, // Nachname Kontaktperson\n  "kontakt_adresse_strasse": string | null, // Straße\n  "kontakt_adresse_hausnummer": string | null, // Hausnummer\n  "kontakt_adresse_plz": string | null, // Postleitzahl\n  "kontakt_telefon": string | null, // Telefon\n  "kontakt_email": string | null, // E-Mail\n  "gespraechsdatum": string | null, // YYYY-MM-DDTHH:MM\n  "kontaktart": LookupValue | null, // Art des Kontakts (select one key: "information" | "beschwerde" | "rueckfrage" | "sonstiges") mapping: information=Information, beschwerde=Beschwerde, rueckfrage=Rückfrage, sonstiges=Sonstiges\n  "gespraechsnotiz": string | null, // Gesprächsnotiz\n  "besonderheiten": string | null, // Besonderheiten\n  "kontakt_adresse_ort": string | null, // Ort\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Kommunikation['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Kommunikation bearbeiten' : 'Kommunikation hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'kontakt_vorname': (
      <div key="kontakt_vorname" className="space-y-1.5">
    ...
    'kontakt_nachname': (
      <div key="kontakt_nachname" className="space-y-1.5">
    ...
    'kontakt_adresse_strasse': (
      <div key="kontakt_adresse_strasse" className="space-y-1.5">
    ...
    'kontakt_adresse_hausnummer': (
      <div key="kontakt_adresse_hausnummer" className="space-y-1.5">
    ...
    'kontakt_adresse_plz': (
      <div key="kontakt_adresse_plz" className="space-y-1.5">
    ...
    'gespraechsdatum': (
      <div key="gespraechsdatum" className="space-y-1.5">
    ...
    'kontaktart': (
      <div key="kontaktart" className="space-y-1.5">
    ...
    'gespraechsnotiz': (
      <div key="gespraechsnotiz" className="space-y-1.5">
    ...
    'kontakt_adresse_ort': (
      <div key="kontakt_adresse_ort" className="space-y-1.5">
  ...
  const FIELD_LABELS: Record<string, string> = {"baustelle": "Baustelle", "kontakt_vorname": "Vorname Kontaktperson", "kontakt_nachname": "Nachname Kontaktperson", "kontakt_adresse_strasse": "Straße", "kontakt_adresse_hausnummer": "Hausnummer", "kontakt_adresse_plz": "Postleitzahl", "kontakt_telefon": "Telefon", "kontakt_email": "E-Mail", "gespraechsdatum": "Datum des Gesprächs", "kontaktart": "Art des Kontakts", "gespraechsnotiz": "Gesprächsnotiz", "besonderheiten": "Besonderheiten", "kontakt_adresse_ort": "Ort"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.KOMMUNIKATION} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
