...
import type { Maengelerfassung, Baustellen, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Maengelerfassung';
...
interface MaengelerfassungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Maengelerfassung['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Omit<Maengelerfassung['fields'], 'mangeltyp' | 'prioritaet' | 'mangel_status'> & {
    mangeltyp?: LookupValue | string;
    prioritaet?: LookupValue | string;
    mangel_status?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  mangeltyp: LOOKUP_OPTIONS['maengelerfassung']?.['mangeltyp'] ?? [],
  prioritaet: LOOKUP_OPTIONS['maengelerfassung']?.['prioritaet'] ?? [],
  mangel_status: LOOKUP_OPTIONS['maengelerfassung']?.['mangel_status'] ?? [],
};
...
export function MaengelerfassungDialog({ open, onClose, onSubmit, defaultValues, recordId, baustellenList, enablePhotoScan = true, enablePhotoLocation = true }: MaengelerfassungDialogProps) {
  const [fields, setFields] = useState<Partial<Maengelerfassung['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Maengelerfassung['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'maengelerfassung');
      await onSubmit(clean as Maengelerfassung['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "baustelle": string | null, // Display name from Baustellen (see <available-records>)\n  "mangeltyp": LookupValue | null, // Mangeltyp (select one key: "falsche_absperrung" | "nacharbeit" | "sicherheitsmangel" | "qualitaetsmangel" | "sonstiges") mapping: falsche_absperrung=Falsche Absperrung, nacharbeit=Nacharbeit erforderlich, sicherheitsmangel=Sicherheitsmangel, qualitaetsmangel=Qualitätsmangel, sonstiges=Sonstiges\n  "mangel_beschreibung": string | null, // Beschreibung des Mangels\n  "prioritaet": LookupValue | null, // Priorität (select one key: "niedrig" | "mittel" | "hoch" | "kritisch") mapping: niedrig=Niedrig, mittel=Mittel, hoch=Hoch, kritisch=Kritisch\n  "meldedatum": string | null, // YYYY-MM-DDTHH:MM\n  "mangel_status": LookupValue | null, // Status (select one key: "offen" | "in_bearbeitung" | "behoben") mapping: offen=Offen, in_bearbeitung=In Bearbeitung, behoben=Behoben\n  "firma_name": string | null, // Verantwortliche Firma\n  "firma_ansprechpartner_vorname": string | null, // Vorname Ansprechpartner Firma\n  "firma_ansprechpartner_nachname": string | null, // Nachname Ansprechpartner Firma\n  "firma_telefon": string | null, // Telefon Firma\n  "mangel_bemerkungen": string | null, // Weitere Bemerkungen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Maengelerfassung['fields']>;
      });
      ...
      if (file && (file.type.startsWith('image/') || file.type === 'application/pdf')) {
        try {
          ...
          setFields(prev => ({ ...prev, mangel_foto: fileUrl }));
        } catch (uploadErr) {
  ...
  const DIALOG_INTENT = defaultValues ? 'Mängelerfassung bearbeiten' : 'Mängelerfassung hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'mangeltyp': (
      <div key="mangeltyp" className="space-y-1.5">
    ...
    'mangel_beschreibung': (
      <div key="mangel_beschreibung" className="space-y-1.5">
    ...
    'mangel_status': (
      <div key="mangel_status" className="space-y-1.5">
    ...
    'firma_name': (
      <div key="firma_name" className="space-y-1.5">
    ...
    'firma_ansprechpartner_vorname': (
      <div key="firma_ansprechpartner_vorname" className="space-y-1.5">
    ...
    'firma_ansprechpartner_nachname': (
      <div key="firma_ansprechpartner_nachname" className="space-y-1.5">
    ...
    'firma_telefon': (
      <div key="firma_telefon" className="space-y-1.5">
    ...
    'mangel_foto': (
      <div key="mangel_foto" className="space-y-1.5">
        <Label htmlFor="mangel_foto">Foto des Mangels</Label>
        {fields.mangel_foto ? (
          <div className="flex items-center gap-3 rounded-lg border p-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm truncate text-foreground">{fields.mangel_foto.split("/").pop()}</p>
              <div className="flex gap-2 mt-1">
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        setFields(f => ({ ...f, mangel_foto: fileUrl }));
                ...
                <button
                  ...
                  onClick={() => setFields(f => ({ ...f, mangel_foto: undefined }))}
                >
                  Entfernen
                </button>
              </div>
    ...
    'mangel_bemerkungen': (
      <div key="mangel_bemerkungen" className="space-y-1.5">
  ...
  const FIELD_LABELS: Record<string, string> = {"baustelle": "Baustelle", "mangeltyp": "Mangeltyp", "mangel_beschreibung": "Beschreibung des Mangels", "prioritaet": "Priorität", "meldedatum": "Meldedatum", "mangel_status": "Status", "firma_name": "Verantwortliche Firma", "firma_ansprechpartner_vorname": "Vorname Ansprechpartner Firma", "firma_ansprechpartner_nachname": "Nachname Ansprechpartner Firma", "firma_telefon": "Telefon Firma", "mangel_foto": "Foto des Mangels", "mangel_bemerkungen": "Weitere Bemerkungen"};
  ...
  return (
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[92vh] flex flex-col overflow-hidden p-0 gap-0 max-sm:[&>button]:size-10 max-sm:[&>button]:grid max-sm:[&>button]:place-items-center max-sm:[&>button>svg]:size-5">
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.MAENGELERFASSUNG} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
