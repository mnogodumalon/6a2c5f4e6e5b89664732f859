...
import type { Fotodokumentation, Baustellen, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Fotodokumentation';
...
interface FotodokumentationDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Fotodokumentation['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Omit<Fotodokumentation['fields'], 'fotokategorie'> & {
    fotokategorie?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  fotokategorie: LOOKUP_OPTIONS['fotodokumentation']?.['fotokategorie'] ?? [],
};
...
export function FotodokumentationDialog({ open, onClose, onSubmit, defaultValues, recordId, baustellenList, enablePhotoScan = true, enablePhotoLocation = true }: FotodokumentationDialogProps) {
  const [fields, setFields] = useState<Partial<Fotodokumentation['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Fotodokumentation['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'fotodokumentation');
      await onSubmit(clean as Fotodokumentation['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "baustelle": string | null, // Display name from Baustellen (see <available-records>)\n  "fotokategorie": LookupValue | null, // Fotokategorie (select one key: "vor_beginn" | "waehrend_arbeiten" | "nach_fertigstellung") mapping: vor_beginn=Vor Beginn der Arbeiten, waehrend_arbeiten=Während der Arbeiten, nach_fertigstellung=Nach Fertigstellung\n  "aufnahmedatum": string | null, // YYYY-MM-DDTHH:MM\n  "foto_beschreibung": string | null, // Beschreibung\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Fotodokumentation['fields']>;
      });
  ...
  const DIALOG_INTENT = defaultValues ? 'Fotodokumentation bearbeiten' : 'Fotodokumentation hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'fotokategorie': (
      <div key="fotokategorie" className="space-y-1.5">
        <Label htmlFor="fotokategorie">Fotokategorie</Label>
  ...
  const FIELD_LABELS: Record<string, string> = {"baustelle": "Baustelle", "fotokategorie": "Fotokategorie", "aufnahmedatum": "Aufnahmedatum und -uhrzeit", "foto": "Foto", "gps_koordinaten": "GPS-Koordinaten", "foto_beschreibung": "Beschreibung"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.FOTODOKUMENTATION} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
